...
const Password = ({ onChange, className, type }) => {
  ...
  return (
    <div className='w-full flex justify-between items-center gap-2'>
      <div
        className={`w-full h-12 bg-slate-700 rounded-xl flex justify-start items-center px-3 relative border-[2px] ${className}`}
      >
        <h2
          className={`absolute font-semibold lg:text-xl duration-500 select-none
                        ${
                          isPasswordFocused
                            ? ' text-accent_green_dark'
                            : ' text-cstm_white hover:cursor-text'
                        }
                        ${
                          isPasswordFocused || !isPasswordEmpty
                            ? 'left-2 -top-8 '
                            : 'left-12 top-[8px]'
                        }
                    `}
        ...
        <input
          ...
          className={`w-full h-12 outline-none bg-transparent lg:text-xl rounded-xl px-3 caret-accent_green_dark 
                        ${
                          isPasswordFocused
                            ? 'text-accent_green_dark selection:text-cstm_bg_dark'
                            : 'text-cstm_white'
                        }
                    `}
        />
