...
const StudySetsPage = () => {
  ...
  const handleDeleteStudySet = async () => {
    ...
    try {
      ...
      console.log(
        `Study sets deleted successfully: ${itemsToDelete.join(', ')}`
      );
    } catch (error) {
    ...
    >
      {showSuccessfullyAdded.value && (
        <Popup
          type='successful'
          title='Success'
          message={'You have successfully added a new study set!'}
          primButtonText='OK'
          onClickPrim={closeAddStudySetSuccessMessage}
        />
      )}
      ...
      {showNotAcceptableFileErrorMessage.value && (
        <Popup
          type='error'
          title='Ooops!'
          message={'The file format you uploaded is not acceptable.'}
          primButtonText='OK'
          onClickPrim={closeNotAcceptableFileErrorMessage}
        />
      )}
      {isStudySetAlreadyExistsActive && (
        <Popup
          type='error'
          title='Ooops!'
          message={'A study set with this name already exists.'}
          primButtonText='OK'
      ...
      <div className='flex flex-col w-full flex-1 py-5 px-5 gap-6 relative z-10 overflow-hidden'>
        ...
        <svg
          className={`absolute z-0 translate-x-[1100px] -translate-y-[320px] scale-[85%] duration-[2000ms]`}
          width='1123'
          height='1128'
          viewBox='0 0 1123 1128'
          fill='none'
